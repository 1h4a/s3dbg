...
export async function POST(request: NextRequest) {
    ...
    if (check.isValid) {
        ...
        try {
            const internal_req: S3Request = constructRequest(data.requestType, config);
            const sdk_command = constructS3Command(internal_req.id, internal_req.config);
        ...
        return NextResponse.json(
            { error: "Could not find request type " + data.requestType + " (" + check.error + ")",  }, { status: 404 }
        )
    }
}
