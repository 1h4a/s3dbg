    ...
    buildInput: (c: CommandConfig): DeleteObjectsCommandInput => {
      ...
      return {
        ...
        Delete: {
          ...
          Objects: [
            { Key: targetKey, ...(versionId ? { VersionId: versionId } : {}) },
          ],
        },
      };
    },
...
export function getMissingCommandSpecIds(): string[] {
  const allRequestIds = s3Requests.flatMap((section) =>
    section.requests.map((r) => r.id),
  );
  return allRequestIds.filter((id) => !(id in commandSpecs));
}

export function constructS3Command(
  id: string,
